...
//#endif
        ...
        private static object ParseString(string strValue, Type t)
        {
            ...
            else if (t == typeof(List<int>))
            {
                ...
                {
                    return res;}
                string[] pairs = strValue.Split(new char[]{
                    '|',','
                });
                foreach (string pair in pairs)
                {
            ...
            }else if (t == typeof(Vector2))
            {
                ...
                float outTest;
                if (!float.TryParse(pairs[0], out outTest))
                {
                    Debug.LogError(("value " + pairs[0] + " is not a float in " +strValue));
                }
                if (!float.TryParse(pairs[1], out outTest))
                {
                    Debug.LogError(("value " + pairs[1] + " is not a float in " +strValue));
                }
            ...
            try
            {
                obj = cv.ConvertFromInvariantString(strValue);
            }
            catch(Exception e)
            {
                Debug.Log("error when parse "+strValue+ " as "+t);
                // ignored
            } 
