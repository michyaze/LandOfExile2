...
public class MageClassSpecialization : ClassSpecialization
{
    public override List<Card> CalculateVictoryDrops(AdventureItemEncounter encounter)
    {
        ...
        //Add two class cards
        for (int ii = 0; ii < 2 + MenuControl.Instance.CountOfCardsInList(MenuControl.Instance.levelUpMenu.extraSpellDropTalent,MenuControl.Instance.levelUpMenu.variableTalentsAcquired); ii += 1)
        {
        ...
        //Add one/2 minion cards
        for (int ii = 0; ii < (MenuControl.Instance.heroMenu.easyMode ? 2 : 1) + MenuControl.Instance.CountOfCardsInList(MenuControl.Instance.levelUpMenu.extraMinionDropTalent, MenuControl.Instance.levelUpMenu.variableTalentsAcquired); ii += 1)
        {
    ...
    public override List<Card> CalculatePurchaseDrops()
    {
        ...
        for (int ii = 0; ii < 3; ii += 1)
        {
            ...
            if (card != null)
            {
                ...
                pathCards.Remove(card);
            }
        }
        for (int ii = 0; ii < 3; ii += 1)
        {
            Card card = ChooseFromElligibleCards(pathCards, true);
            if (card != null)
            {
                cardsToShow.Add(card);
                pathCards.Remove(card);
        ...
        if (Random.Range(1, 101) <= 35)
        {
            ...
            {
                cardsToShow.Add(ChooseFromElligibleCards(classCards, true));
            }
        }
    ...
    public override List<Card> CalculateChestDrops()
    {
        ...
        //1x Level 3 Path Card
        List<Card> elligibleCards = MenuControl.Instance.heroMenu.FilterCardsOfLevel(MenuControl.Instance.heroMenu.heroPath.pathCards, 3);
        elligibleCards.AddRange(MenuControl.Instance.heroMenu.FilterCardsOfLevel(MenuControl.Instance.heroMenu.GetUnlockedMinionCards(), 3));
        cardsToShow.Add(ChooseFromElligibleCards(elligibleCards, true));

        //1x Level 2 Class Card
        elligibleCards = MenuControl.Instance.heroMenu.FilterCardsOfLevel(MenuControl.Instance.heroMenu.heroClass.classCards, 2);
        elligibleCards.AddRange(MenuControl.Instance.heroMenu.FilterCardsOfLevel(MenuControl.Instance.heroMenu.GetUnlockedSpellCards(), 2));
        cardsToShow.Add(ChooseFromElligibleCards(elligibleCards, true));
